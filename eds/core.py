...
class Group(Object):
    ...
    def __getitem__(self, name):
        ...
        if not _is_valid_object_folder(folder):
            raise IOError("Folder '" + folder + "' is not a valid EDS object.")
...
class File(Group):
    def __init__(self, folder, mode=None):
        ...
        if already_exists:
            if not _is_valid_object_folder(folder):
                raise FileExistsError("Path '" + folder + "' already exists, but is not a valid EDS object.")
            if self.meta["type"] != "File":
                raise FileExistsError("Path '" + folder + "' already exists, but is not a valid EDS file.")
